...
class TIS_GrabberDLL(object):
    if sys.maxsize > 2 ** 32:
        __tisgrabber = C.windll.LoadLibrary(f"{root}/tisgrabber_x64.dll")
    else:
        __tisgrabber = C.windll.LoadLibrary(f"{root}/tisgrabber.dll")
...
class TIS_CAM(object):
    ...
    def SetFrameReadyCallback(self, CallbackFunction, data):
        ...
        """
        return TIS_GrabberDLL.SetFrameReadyCallback(self._handle, TIS_GrabberDLL.FRAMEREADYCALLBACK(CallbackFunction), data)
